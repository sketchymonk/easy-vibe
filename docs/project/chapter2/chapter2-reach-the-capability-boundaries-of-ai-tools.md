...
课后，你需要完成一份关于 AI 工具的调查报告。对于不同赛道的小组，每个小组必须提交一份相对全面的报告。

报告格式可以参考这个：[Vibe Coding Tools] 贪吃蛇游戏教程

在报告中，你需要根据今天赛道设计不断完善内容。通过“如何找到最好的模型”中的大模型竞技场排名，识别其他不同的工具（如果你在 AI 编码组，你可以参考 Yerim 的比较来完成作业），并对其生成效果进行比较。比较应从功能、生成质量、生成速度或价格等方面进行多样化。
